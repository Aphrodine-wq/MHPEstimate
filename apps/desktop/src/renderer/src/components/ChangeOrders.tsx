...
export function ChangeOrders({ estimateId, estimateStatus, onTotalChanged }: ChangeOrdersProps) {
  ...
  const fetchChangeOrders = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to load change orders. Please try again.", err);
    } finally {
      setLoading(false);
    }
  }, [estimateId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(parsedCost)) {
      console.error("Cost impact must be a valid number");
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        const { error } = await supabase
          ...
          .eq("id", editingId);
        if (error) throw error;
        console.log("Change order updated");
      } else {
        ...
        if (error) throw error;
        console.log(`Change Order #${nextNumber} created`);
      }

      resetForm();
      await fetchChangeOrders();
    } catch (err) {
      console.error(editingId ? "Failed to update change order. Please try again." : "Failed to create change order. Please try again.", err);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleUpdateStatus = async (id: string, newStatus: "approved" | "rejected") => {
    ...
    try {
        ...
        if (co) {
          ...
          if (!fetchErr && estimateData) {
            ...
            const { error: updateErr } = await supabase
              ...
              .eq("id", estimateId);
            if (updateErr) {
              console.error("Change order approved but failed to update estimate total.", updateErr);
            } else {
              onTotalChanged?.();
            }
          }
      ...
      console.log(`Change order ${newStatus}`);
      await fetchChangeOrders();
    } catch (err) {
      console.error(`Failed to ${newStatus} change order. Please try again.`, err);
    } finally {
      setUpdatingId(null);
    }
  };
  ...
  const handleToggleClientSigned = async (co: EstimateChangeOrder) => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .eq("id", co.id);
      if (error) throw error;
      console.log(newSigned ? "Marked as client signed" : "Signature removed");
      await fetchChangeOrders();
    } catch (err) {
      console.error("Failed to update signature status. Please try again.", err);
    } finally {
      setUpdatingId(null);
    }
  };
  ...
  const handleDelete = async (co: EstimateChangeOrder) => {
    ...
    try {
      const { error } = await supabase
        ...
        .eq("id", co.id);
      if (error) throw error;
      console.log(`Change Order #${co.change_number} deleted`);
      await fetchChangeOrders();
    } catch (err) {
      console.error("Failed to delete change order. Please try again.", err);
    } finally {
      setDeletingId(null);
    }
  };
                ...
                {co.status === "pending" && (
                  <>
                    <button
                      ...
                      className="rounded-md bg-[var(--green)] px-3 py-1.5 text-[11px] font-medium text-white transition-all hover:bg-[var(--accent-hover)] disabled:opacity-50"
                    >
                      Approve
                    </button>
                    <button
                      ...
                      className="rounded-md bg-[var(--red)] px-3 py-1.5 text-[11px] font-medium text-white transition-all hover:bg-[var(--accent-hover)] disabled:opacity-50"
                    >
                      Reject
                    </button>
                    <button
