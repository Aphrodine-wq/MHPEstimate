...
export function PlanImport({ onImport, projectType = "General" }: PlanImportProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Plan analysis requires the web version (API endpoint not available in desktop)
      setError("Plan analysis with AI is currently only available in the web version. The desktop app supports file upload and PDF rendering, but the AI analysis endpoint requires the web server.");
      setStep("error");
    } catch (err) {
