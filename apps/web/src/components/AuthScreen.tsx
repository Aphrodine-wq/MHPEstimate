...
export function AuthScreen({ onAuthenticated, initialView = "login", onDevBypass }: AuthScreenProps) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (data.user) {
      ...
      if (memberError) {
        console.error("Failed to create team member record:", memberError);
        // Don't block authentication — the record can be created later
      }
    }
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-[400px] px-6">
        ...
        {/* Card */}
        <div className="rounded-2xl bg-white p-6 shadow-[0_2px_20px_rgba(0,0,0,0.06)]">
          {error && (
