  ...
  const handleApproveChangeOrder = useCallback(
    async (changeOrderId: string) => {
      ...
      try {
        ...
        const json = (await res.json()) as { success?: boolean; error?: string };
        if (!res.ok || !json.success) {
          alert(json.error ?? "Failed to approve change order. Please try again.");
          return;
        }
        // Update local data
        setData((prev) =>
          prev
            ? {
                ...
                changeOrders: prev.changeOrders.map((co) =>
                  co.id === changeOrderId ? { ...co, client_signed: true } : co,
                ),
              }
            : prev,
        );
        ...
        <div style={{ maxWidth: 896, margin: "0 auto" }}>
          ...
          {changeOrders.length > 0 && (
            ...
            >
              ...
              >
                {["#", "Description", "Timeline", "Cost Impact", "Action"].map((h, i) => (
                  <span
                    key={h}
                    style={{
                      fontSize: 10,
                      ...
                      textTransform: "uppercase",
                      color: "rgba(255,255,255,0.85)",
                      textAlign: i === 4 ? "right" : (i >= 2 ? "right" : "left"),
                    }}
                  >
                    {h}
                  </span>
                ...
                >
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, justifyContent: "center" }}>
                    {!co.client_signed && (
                      ...
                      >
                        {approvingCoId === co.id ? "Approving…" : "Approve"}
                      </button>
                    )}
                  </div>
                </div>
